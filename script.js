...
const chat = $("chat"), chatForm = $("chatForm"), messageInput = $("messageInput"), sendButton = $("sendButton"), connectionStatus = $("connectionStatus"), avatar = document.querySelector("#chatScreen #avatar") || $("avatar"), quotaText = $("quotaText");
const voiceButton = $("voiceButton"), stopVoiceButton = $("stopVoiceButton"), voiceHint = $("voiceHint");
...
function stopPixPolling(message = "Verificação interrompida.") {
  if (pixPollingTimer) clearInterval(pixPollingTimer);
  pixPollingTimer = null;
  currentPaymentId = null;
  if (pixStatusText && message) pixStatusText.textContent = message;
}

function showPendingPix(pix) {
  if (!pix || !pix.paymentId) return false;
  currentPaymentId = pix.paymentId;
  if (pix.qrCodeBase64) pixQrImage.src = `data:image/png;base64,${pix.qrCodeBase64}`;
  pixCopyCode.value = pix.qrCode || "";
  pixCard.classList.remove("hidden");
  pixStatusText.textContent = "Pix pendente recuperado. Aguardando pagamento...";
  return true;
}

async function savePendingPix(pix) {
  if (!currentUser) return;
  profile = { ...(profile || {}), pendingPix: pix };
  await setDoc(userRef(), { pendingPix: pix }, { merge: true });
}

async function clearPendingPix() {
  if (!currentUser) return;
  profile = { ...(profile || {}), pendingPix: null };
  await setDoc(userRef(), { pendingPix: null }, { merge: true });
}

async function restorePendingPixIfNeeded() {
  const pending = profile?.pendingPix;
  if (!pending?.paymentId || hasActiveAccess()) return;
  if (showPendingPix(pending)) {
    await startPixPolling(pending.paymentId, { keepCard: true, fromRestore: true });
  }
}

async function startPixPolling(paymentId, options = {}) {
  stopPixPolling(null);
  currentPaymentId = paymentId;
  pixStatusText.textContent = options.fromRestore ? "Verificando Pix pendente..." : "Aguardando pagamento...";
  ...
  const check = async () => {
    ...
    try {
      ...
      if (data.status === "approved") {
        const paidPaymentId = currentPaymentId;
        stopPixPolling(null);
  ...
  await check();
  if (currentPaymentId) pixPollingTimer = setInterval(check, 5000);
}
...
function speak(text) {
  ...
  utterance.rate = 1;
  utterance.pitch = 1;
  utterance.onstart = () => setTedState("talking", "Ted está falando...");
  utterance.onend = () => setTedState(null, "Pronto para conversar");
  utterance.onerror = () => setTedState(null, "Pronto para conversar");
...
async function sendMessage(raw) {
  ...
  setTedState("thinking", "Ted está pensando...");
  ...
  try {
    ...
    if (!("speechSynthesis" in window) || !window.speechSynthesis.speaking) setTedState(null, "Pronto para conversar");
    messageInput.focus();
  }
}
...
onAuthStateChanged(auth, async (user) => {
  ...
  updateAccessUI();
  renderHome();
  if (!hasActiveAccess() && profile?.pendingPix?.paymentId) {
    showScreen("pay");
    await restorePendingPixIfNeeded();
  } else {
    showScreen("home");
  }
});
...
function startListening() {
  ...
  if (!SpeechRecognition) { alert("Seu navegador não suporta reconhecimento de voz. Tente pelo Chrome no Android."); return; }
  if (isSending) return;
  ...
  if (voiceHint) voiceHint.textContent = "Pode falar. Quando você terminar, o Ted responde.";
  setTedState("listening", "Ted está ouvindo...");
  ...
  recognition.onerror = () => {
    if (voiceHint) voiceHint.textContent = "Não consegui ouvir bem. Toque no microfone e tente de novo.";
  };

  recognition.onend = () => {
    isListening = false;
    voiceButton.textContent = "🎤 Falar com Ted";
    if (!isSending && !("speechSynthesis" in window && window.speechSynthesis.speaking)) {
      setTedState(null, "Pronto para conversar");
    }
  };
...
pixForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    await savePendingPix(pendingPix);
    showPendingPix(pendingPix);
...
cancelPixButton.addEventListener("click", async () => {
  stopPixPolling("Compra cancelada. Você pode gerar um novo Pix quando quiser.");
  await clearPendingPix();
  pixCard.classList.add("hidden");
...
window.addEventListener("beforeunload", () => { if (pixPollingTimer) clearInterval(pixPollingTimer); });

if (voiceHint) {
  const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
  if (!SpeechRecognition) voiceHint.textContent = "Reconhecimento de voz indisponível neste navegador. Use o Chrome para falar com o Ted.";
}
