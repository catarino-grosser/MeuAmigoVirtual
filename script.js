...
function pendingPixStorageKey() {
  return currentUser ? `ted_pending_pix_${currentUser.uid}` : "ted_pending_pix";
}

function renderPendingPix(pix) {
  if (!pix || !pix.payment_id) return;
  pixQrImage.src = `data:image/png;base64,${pix.qr_code_base64 || ""}`;
  pixCopyCode.value = pix.qr_code || "";
  pixCard.classList.remove("hidden");
}

async function savePendingPix(pix) {
  if (!currentUser || !pix?.payment_id) return;
  const data = {
    ...
    amount: 1,
    createdAtMs: pix.createdAtMs || nowMs(),
    updatedAt: serverTimestamp()
  };

  try { localStorage.setItem(pendingPixStorageKey(), JSON.stringify(data)); } catch {}

  await setDoc(doc(db, "users", currentUser.uid, "payments", "pendingPix"), data, { merge: true });
}

async function loadPendingPix() {
  if (!currentUser) return null;

  let localData = null;
  try {
    const raw = localStorage.getItem(pendingPixStorageKey());
    localData = raw ? JSON.parse(raw) : null;
  } catch {}

  try {
    const snap = await getDoc(doc(db, "users", currentUser.uid, "payments", "pendingPix"));
    if (snap.exists()) return { ...localData, ...snap.data() };
  } catch (error) {
    console.warn("Não consegui ler Pix pendente no Firestore:", error);
  }
...
async function clearPendingPix() {
  if (!currentUser) return;
  try { localStorage.removeItem(pendingPixStorageKey()); } catch {}
  try { await deleteDoc(doc(db, "users", currentUser.uid, "payments", "pendingPix")); } catch (error) { console.warn("Não consegui limpar Pix pendente:", error); }
}

async function resumePendingPix() {
  const pending = await loadPendingPix();
  if (!pending?.payment_id) return false;

  renderPendingPix(pending);
  pixStatusText.textContent = "Retomando verificação do Pix pendente...";
  await startPixPolling(pending.payment_id);
  return true;
}
...
onAuthStateChanged(auth, async (user) => {
  ...
  updateAccessUI();
  renderHome();
  const resumedPix = await resumePendingPix();
  showScreen(resumedPix && !hasActiveAccess() ? "pay" : "home");
});
