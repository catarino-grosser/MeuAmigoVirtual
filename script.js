...
async function loadProfile() {
  ...
  if (snap.exists()) {
    ...
    profile = { name: currentUser.displayName || "amigo", email: currentUser.email, createdAt: serverTimestamp(), plan: "free" };
...
authForm.addEventListener("submit", async (event) => {
  ...
  try {
    if (isRegisterMode) {
      ...
      await setDoc(doc(db, "users", cred.user.uid), { name, email, createdAt: serverTimestamp(), plan: "free" }, { merge: true });
...
$("openChatButton").addEventListener("click", async () => { showScreen("chat"); await loadMessages(); await loadDailyUsage(); messageInput.focus(); });
